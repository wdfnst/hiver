/*
 ...
 modified by : Weidong, ZHANG. mail:wdfnst@pku.edu.cn
 Version     : 0.2
 ============================================================================
 */
#include <stdio.h>
...
public:
    std::string calc(const char *pathname)
    {
        FILE *fp = NULL;
        ...
        md5();
        fclose(fp);
        //printf("%s-->is MD5=", pathname);
        //高低位逆反输出
        //printf("%08x%08x%08x%08x\n",PP(A),PP(B),PP(C),PP(D));
        char retVal[32];
        snprintf(retVal, 32, "%08x%08x%08x%08x", PP(A), PP(B), PP(C), PP(D));
